...
const Dashboard: React.FC<DashboardProps> = ({ summary, lastUpdated }) => {
  const statusConfig = {
    ...
    alert: {
      ...
      label: "WASPADA",
      description:
        "Aktivitas seismik kecil atau perubahan cuaca terdeteksi. Tetap terinformasi.",
    },
    danger: {
      color:
      ...
      label: "BAHAYA",
      description:
        "Risiko bencana signifikan terdeteksi. Ikuti protokol darurat setempat.",
    },
  };
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end">
            <span className="text-sm text-slate-500 dark:text-slate-400 font-medium">
              Bencana Lokal (24 Jam)
            </span>
            <span className="text-2xl font-black text-slate-900 dark:text-white">
              {summary.activeDisastersCount}
            </span>
          </div>
          <div className="w-full bg-slate-100 dark:bg-slate-800 h-1.5 rounded-full overflow-hidden">
            <div
              ...
              style={{
                width: `${Math.min(100, (summary.activeDisastersCount / 5) * 100)}%`,
              }}
            />
          </div>
          <p className="text-[9px] text-slate-400 dark:text-slate-500 mt-2 italic">
            *Laporan bencana aktif di sekitar radius Anda (Gempa, Banjir, dll)
          </p>
        </div>
      </div>
    </div>
