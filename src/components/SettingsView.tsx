...
const SettingsView: React.FC<SettingsViewProps> = ({ settings, onSave }) => {
  ...
  const handleSearch = async () => {
    ...
    if (result) {
      ...
      setGeocodingResult({
        success: true,
        message: `Lokasi ditemukan: ${result.name}`,
      });
    } else {
      setGeocodingResult({
        success: false,
        message: "Lokasi tidak ditemukan. Coba nama wilayah lain.",
      });
    }
  ...
  const handleSave = () => {
    if (
      ...
      setGeocodingResult({
        success: false,
        message: "Harap cari dan pilih lokasi terlebih dahulu.",
      });
      return;
    }
      ...
      <section className="bg-white dark:bg-slate-800 rounded-2xl p-6 border border-slate-200 dark:border-slate-700 shadow-sm">
        ...
        <div className="space-y-6">
          <div className="flex p-1 bg-slate-100 dark:bg-slate-900 rounded-xl">
            <button
              onClick={() =>
                setLocalSettings({ ...localSettings, locationMode: "gps" })
              }
            ...
            <button
              onClick={() =>
                setLocalSettings({ ...localSettings, locationMode: "manual" })
              }
              className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-xs font-bold transition-all ${
                localSettings.locationMode === "manual"
                  ? "bg-white dark:bg-slate-800 text-blue-600 shadow-sm"
            ...
            <div className="space-y-4 animate-in fade-in slide-in-from-top-2 duration-300">
              <div>
                <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5">
                  Wilayah (Kelurahan, Kecamatan, Kota)
                </label>
              ...
              {localSettings.manualLocation && (
                <div className="p-4 bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-100 dark:border-slate-800">
                  ...
                  <p className="text-sm font-bold text-slate-900 dark:text-white">
                    {localSettings.manualLocation.name}
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-slate-500">
                Terima peringatan real-time untuk bencana baru.
              </p>
            </div>
            <button
              onClick={() =>
